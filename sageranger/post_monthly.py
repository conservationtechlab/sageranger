...
This script defines a function called post_monthly_obs where an http request is used to retrieve the subject IDs of all camera traps included in sagebrush,
and uses an http request to post an empty observation with longitude and latitude (0,0) once every month with cougar vision to ensure the camera stays 
active on sagebrush.
'''
...
def post_monthly_obs(token, auth):
    ...
    '''post_monthly_obs
    This function gets the subject id of camera traps and posts observations onto earthranger
    Args: 
	token: the token for api calls in earthranger 'str'
	auth: another token for ER api calls as specified in config yml 'str'
